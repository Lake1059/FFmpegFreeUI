Public Class 视频编码器数据库_v6
    ...
    Private Shared Sub 初始化分类()
        ...
        加入分类(预设数据_v6.视频编码器类型.视频, "AV2", "FFmpeg 暂无可用编码器。", "av2", "avmenc")
        加入分类(预设数据_v6.视频编码器类型.视频, "H.266/VVC", "新一代高压缩标准；目前仅 libvvenc，编码开销高。", "libvvenc")
        加入分类(预设数据_v6.视频编码器类型.视频, "AV1", "高压缩格式，适合存储；软件编码较慢，支持时优先硬件编码。", "libaom-av1", "libsvtav1", "av1_nvenc", "av1_qsv", "av1_amf", "av1_d3d12va", "librav1e", "av1_vulkan")
        加入分类(预设数据_v6.视频编码器类型.视频, "H.265/HEVC", "通用高压缩格式；追求压缩效率时优先 x265 软件编码。", "libx265", "hevc_nvenc", "hevc_qsv", "hevc_amf", "hevc_d3d12va", "hevc_vulkan", "libkvazaar", "libsvt_hevc")
        加入分类(预设数据_v6.视频编码器类型.视频, "H.264/AVC", "兼容性优先；追求压缩效率时建议 HEVC/AV1，软件编码选 libx264。", "libx264", "libx264rgb", "libopenh264", "h264_nvenc", "h264_qsv", "h264_amf", "h264_d3d12va", "h264_vulkan")
        加入分类(预设数据_v6.视频编码器类型.视频, "ProRes", "高码率剪辑中间格式，不适合最终存储。", "prores_ks", "prores_aw")
        加入分类(预设数据_v6.视频编码器类型.视频, "VP9 # VP8", "Google 视频格式；用于 WebM 或既有工作流。", "libvpx-vp9", "libsvt_vp9", "vp9_qsv", "libvpx")
        加入分类(预设数据_v6.视频编码器类型.视频, "FFv1", "无损存档格式，体积大，适合长期归档。", "ffv1 -level 3", "ffv1 -level 1", "ffv1_vulkan")
        加入分类(预设数据_v6.视频编码器类型.视频, "其他现代编码", "较新或生态较弱的编码实现，兼容性和效率因实现而异。", "libxeve", "libxavs", "libxavs2", "libuavs3e", "liboapv")
        加入分类(预设数据_v6.视频编码器类型.视频, "老旧编码", "仅用于旧设备或旧格式兼容。", "mpeg4", "libxvid", "rv20", "rv10", "wmv2", "wmv1")

...
联网有三种状态。禁用联网时没有联网工具。端点联网使用模型端点的原生联网能力，通常走 `web_search` 并由端点实现。 本地联网由 3FUI 在本机发起 HTTP 请求，`web_search` 会抓取搜索页文本，`fetch_url` 可读取指定 URL 的网页文本。联网结果不等于权威事实，高风险或时效性信息仍应标注来源和不确定性。
...
PowerShell 优先用于只读检查、诊断和用户明确要求的命令。删除、覆盖、批量修改、安装软件、修改系统设置、联网下载执行、停止进程等高风险操作需要用户明确确认。执行命令后必须检查返回的 exit_code、stdout、stderr 和实际效果，不要只凭命令看起来正确就回答完成。"
    End Function
...
`输出_自动命名选项` 中，默认 `附加_递增时间戳` 会替换文件名结尾已有的 `_yyyy.MM.dd-HH.mm.ss`。`附加_2位结尾序号` 和 `附加_3位结尾序号` 始终从 `01` 或 `001` 开始递增，判断已占用序号时忽略扩展名，不自带空格或下划线；需要分隔符就写入 `输出命名_结尾文本`。
...
视频和音频编码器来自数据库。禁用会生成 `-vn` 或 `-an`。复制流遇到来自滤镜的输出时，不会再对滤镜输出写复制编码。视频附加参数按输出流选择器写入预设、profile、场景优化、图片质量、`-r`、`-pix_fmt`、色彩元数据、`-gpu`、`-threads` 和质量参数。
...
合并工具通常按文件列表和输出路径工作。混流工具的 files 可以是对象数组，适合描述不同输入文件的流选择、语言、标题、默认轨等。抽流工具使用 `file`、`output_location`、`selected_streams`，并可在运行时决定是否强制自动命名。

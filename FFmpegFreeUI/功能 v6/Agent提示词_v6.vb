...
3. 编码队列中的任务拥有自己的预设副本。修改参数面板不会自动改变已经加入队列的任务；可以用 get_queue_summary 读取全部或指定任务信息，用 control_queue_tasks 开始、暂停、恢复、停止、移除或重置任务；只有用户明确要求同步队列时，才能调用 sync_parameter_panel_to_queue，并且只会同步尚未开始的预设任务。
...
权限和联网：
1. 权限级别分为安全区域、环境控制、系统访问。安全区域只开放参数面板读取、字段查询和参数面板修改；环境控制开放页面切换、队列信息读取和控制、准备文件、集成工具、硬件概要、参数预设和控件文本读取；系统访问额外开放本地文件读取、目录列举、图片信息和 PowerShell。
...
3. 停止或移除编码队列任务，尤其是 target=all，只能在用户明确要求时执行。
4. 任务受阻时向用户报告具体阻塞点，不要无限重试。"
        End Get
    End Property

...
#Region "SP Agent 端点"
    ...
    Private Shared ReadOnly SPAgent端点内存列表 As New List(Of AgentSpEndpointInfo)
    ...
    Public Shared Sub 启动时后台获取SPAgent端点()
        ...
        Task.Run(Async Function()
                     Try
                         ...
                         If 校验错误 <> "" Then
                             设置SPAgent端点获取结果(New List(Of AgentSpEndpointInfo), 校验错误)
                             Return
                         End If

                         Dim 端点列表 As List(Of AgentSpEndpointInfo)
                     ...
                     Catch ex As Exception
                         设置SPAgent端点获取结果(New List(Of AgentSpEndpointInfo), 获取异常消息(ex))
                     End Try
                 End Function)
    End Sub
    ...
    Private Shared Function 校验SPAgent端点获取资格() As String
        ...
        Dim 实际SHA256 As String = 计算文件SHA256(解锁器路径)
    ...
    Private Shared Function 计算文件SHA256(filePath As String) As String
    ...
    Private Shared Function 安全获取程序集类型(程序集 As Reflection.Assembly) As IEnumerable(Of Type)
        Try
            Return 程序集.GetTypes()
    ...
    Private Shared Async Function 获取SPAgent端点列表数据Async(cancellationToken As CancellationToken) As Task(Of List(Of AgentSpEndpointInfo))
        Using request As New HttpRequestMessage(HttpMethod.Get, SPAgent端点列表地址)
            ...
            Using response = Await SPAgent端点Http.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(False)
                ...
                Dim 端点列表 = JsonSerializer.Deserialize(Of List(Of AgentSpEndpointInfo))(raw, JsonSO)
                Return 清理SPAgent端点列表(端点列表)
            End Using
        End Using
    End Function

    Private Shared Function 清理SPAgent端点列表(端点列表 As IEnumerable(Of AgentSpEndpointInfo)) As List(Of AgentSpEndpointInfo)
        Dim result As New List(Of AgentSpEndpointInfo)
        ...
        For Each item In 端点列表
            ...
            Try
                Dim unused = AgentEndpointClient.ParseAdditionalHeaders(item.ExtraHeaders)
            Catch
                Continue For
            End Try
            Try
            ...
            result.Add(New AgentSpEndpointInfo With {

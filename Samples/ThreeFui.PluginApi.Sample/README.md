...
```powershell
dotnet build .\Samples\ThreeFui.PluginApi.Sample\ThreeFui.PluginApi.Sample.csproj -c Release
```

只把生成的 `ThreeFui.PluginApi.Sample.3fui.dll` 放到 3FUI 的 `Plugin` 目录。不要复制构建目录中的

...
3FUI 的底层逻辑就是基于预设的，可以很方便地与他人分享方案，导出的预设自行管理；多数地方直接标出参数名以及准确显示 ffmpeg 输出的信息，更易于上手和尝试新方案；实时计算剩余时间、预估最终大小、可暂停任务、最多自动同时开始 10 个任务；不会向输出文件里写入软件信息；没做用户设置也懒得做，缓存自动收拾，不碰注册表，不在任何地方扔垃圾（崩溃转储除外，这不是我能控制的），也不会收集任何信息。
